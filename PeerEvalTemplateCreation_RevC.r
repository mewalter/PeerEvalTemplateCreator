...
TwoPerson <- data %>% mutate(TeamNum = gsub(" ", "",str_extract(data$Project.Number,"^([^-])+"))) %>% filter(Number.Members==2) 
ThreePerson <- data %>% mutate(TeamNum = gsub(" ", "",str_extract(data$Project.Number,"^([^-])+"))) %>% filter(Number.Members==3) 
FourPerson <- data %>% mutate(TeamNum = gsub(" ", "",str_extract(data$Project.Number,"^([^-])+"))) %>% filter(Number.Members==4) 
FivePerson <- data %>% mutate(TeamNum = gsub(" ", "",str_extract(data$Project.Number,"^([^-])+"))) %>% filter(Number.Members==5)
SixPerson <- data %>% mutate(TeamNum = gsub(" ", "",str_extract(data$Project.Number,"^([^-])+"))) %>% filter(Number.Members==6)
SevenPerson <- data %>% mutate(TeamNum = gsub(" ", "",str_extract(data$Project.Number,"^([^-])+"))) %>% filter(Number.Members==7) 
